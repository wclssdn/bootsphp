...
class Response {
	...
	/**
	 * 发送文件
	 * @param string $filename 文件名, 可以是具体路径. 可以是下载文件名
	 * @param string $content 输出的内容. 如果文件名给定的是下载文件具体路径, 则此参数为空即可
	 * @param string $saveAs 下载的文件名 默认为$filename的文件名
	 */
	public function sendFile($filename, $content = '', $saveAs = ''){
		$mime = $this->getMime($filename);
		$this->setContentType($mime);
		$isFile = is_file($filename);
		$saveAs || $saveAs =\pathinfo($filename, PATHINFO_BASENAME);
		$ua = $_SERVER['HTTP_USER_AGENT'];
		if (preg_match('/MSIE/', $ua)){
			$this->setHeader('Content-Disposition', array(
				'attachment', 
				'filename' => rawurlencode($saveAs)));
		}else if (preg_match('/Firefox/', $ua)){
			$this->setHeader('Content-Disposition', array(
					'attachment',
					'filename*' => "\"utf8''{$saveAs}\""));
		}else{
			$this->setHeader('Content-Disposition', array(
				'attachment', 
				'filename' => $saveAs));
		...
		if ($content){
			...
			if (!$this->isModifyed(filemtime($filename))){
				$this->setStatus(self::STATUS_NOT_MODIFIED);
			}
			$this->setLastModify(filemtime($filename));
			readfile($filename);
		}else{
			echo '';
		}
